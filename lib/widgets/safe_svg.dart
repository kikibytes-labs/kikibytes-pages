  ...
  static Future<String> _loadAndCache(String assetName) {
    ...
    return _inflight.putIfAbsent(normalized, () async {
      ...
      try {
        ...
        // maybe asset was mis‑named; try png if original was svg
        if (normalized.toLowerCase().endsWith('.svg')) {
          final alt = normalized.replaceAll(RegExp(r'\.svg\$'), '.png');
          final bytes = await rootBundle.load(alt);
          list = bytes.buffer.asUint8List();
          loadedPath = alt;
        } else {
          rethrow;
